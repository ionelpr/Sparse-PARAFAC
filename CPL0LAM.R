...
# source("LMpenL0.R")
# 
# CPL0LAM <- function(X, n, m, p, r, lamA, lamB, lamC, conv, maxit){
...
#     y_A <- as.matrix(c(X))
#     X_A <- t(H1) %x% diag(n)
#     updA <- lm.l0(y_A, X_A, lamA, 1e-3, FALSE)
#     A <- matrix(updA$beta, n, r)
...
#     X_B <- t(H1) %x% diag(m)
#     updB <- lm.l0(y_B, X_B, lamB, 1e-3, FALSE)
#     B <- matrix(updB$beta, m, r)
...
#     X_C <- t(H1) %x% diag(p)
#     updC <- lm.l0(y_C, X_C, lamC, 1e-3, FALSE)
#     C <- matrix(updC$beta, p, r)  
...
#   return(list(A = A, B = B, C = C, f = f, fp = fp, cputime = cputime, lamA = lamA, lamB = lamB, lamC = lamC))  
# }
...
CPL0LAM <- function(X, n, m, p, r, lamA, lamB, lamC, conv, maxit, start, A = NULL, B = NULL, C = NULL){
  ...
  cputime <- system.time({
    ...
    #temp <- CPfunc(X, n, m, p, r, 1, 1, 1, 1, 1e-6, 5)
